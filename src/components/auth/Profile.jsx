...
import { deleteUserById, getBookingsByUserId, getUser } from "../utils/ApiFunctions"
...
const Profile = () => {
	...
	useEffect(() => {
		getUser(userId, token)
			...
			.catch((error) => {
				...
				setErrorMessage(`Failed to retrieve user: ${errorMessage}`);
			})
		setTimeout(() => {
			setErrorMessage("");
		}, 4000)
	...
	useEffect(() => {
		getBookingsByUserId(userId, token)
			...
			.catch((error) => {
				setErrorMessage(`Failed to retrieve bookings: ${error.message}`);
			})
		setTimeout(() => {
			setErrorMessage("");
		}, 4000)
	...
	async function handleAccountDeletion() {
		...
		const confirmed = window.confirm("Are you sure you want to delete the account? This action cannot be undone");
					...
					<div className="card-body">
						<div className="col-md-10 mx-auto">
							...
							<div className="d-flex justify-content-center">
								<div className="mx-2">
									<button className="btn btn-danger btn-sm" onClick={() => handleAccountDeletion()}>
										Close account
									</button>
								</div>
							</div>
